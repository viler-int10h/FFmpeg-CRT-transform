...
:: parameter 2 = input image
:: parameter 3 = output image
...
if [%1] neq [] if [%2] neq [] goto :ARGS_OK
	echo. & echo USAGE:    %~n0 configFile inputImage [outputImage]
	echo. & echo   If outputImage is omitted, the input filename will be appended with
...
echo. & echo Couldn't get valid width/height for input image "%2"^^^! & exit /b
:ALL_OK
...
if /i "%SCANLINES_ON%"=="yes" (
	...
	if /i "%BLOOM_ON%"=="yes" (
		...
		ffmpeg -hide_banner -y^
		-i TMPscanlines.png -i TMPstep02.png^
		-filter_complex ^"^
		...
		ffmpeg -hide_banner -y^
		-i TMPstep02.png -i TMPbloom.png -i TMPshadowmask.png -i TMPcorners.png^
		-filter_complex ^"^
		...
		ffmpeg -hide_banner -y^
		-i TMPstep02.png -i TMPscanlines.png -i TMPshadowmask.png -i TMPcorners.png^
		-filter_complex ^"^
			[0][1]blend=all_mode='multiply':all_opacity=%SL_ALPHA%[a],^
			[a][2]blend=all_mode='multiply':all_opacity=%OVL_ALPHA%[b],^
			[b][3]overlay=0:0:format=rgb,^
	...
	ffmpeg -hide_banner -y^
	-i TMPstep02.png -i TMPshadowmask.png -i TMPcorners.png^
	-filter_complex ^"^
...
ffmpeg -hide_banner -y -i TMPstep03.png -vf ^"^
	crop=%CROP_STR%,^
	lutrgb='r=gammaval(2.2):g=gammaval(2.2):b=gammaval(2.2)',^
